...
def build_digests(
    ...
    papers_classified: dict[str, list[dict[str, Any]]] | None = None,
) -> list[Path]:
    """
    ...
    Args:
        ...
        papers_classified: Output of classifier.classify_papers() (optional)
    ...
    for profile_key, models in classified.items():
        ...
        papers = (papers_classified or {}).get(profile_key, [])
        ...
        logger.info("Wrote digest: %s (%d models, %d papers)", md_path, len(models), len(papers))
...
"""
    ...
    if papers:
        ...
        for paper in papers:
            ...
            date = published.strftime("%Y-%m-%d") if published and hasattr(published, "strftime") else "?"
            abstract = paper.get("abstract", "")
            abstract_preview = abstract[:300] + "…" if len(abstract) > 300 else abstract
            categories = ", ".join(paper.get("categories", []))

            papers_section += f"### [{title}]({url})\n"
            papers_section += f"**Authors:** {author_str}  \n"
            papers_section += f"**Published:** {date}  \n"
            papers_section += f"**Categories:** `{categories}`  \n\n"
            papers_section += f"{abstract_preview}\n\n"
...
def _paper_to_json(paper: dict[str, Any]) -> dict[str, Any]:
    """Prepare paper dict for JSON serialisation (handle datetime etc.)."""
    out = {}
